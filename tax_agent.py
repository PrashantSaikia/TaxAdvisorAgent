...
class TaxCalculator:
    ...
    def get_tax_optimizations(self, user: UserDetails, calculation: Dict) -> List[Dict]:
        ...
        current_pension = calculation['pension_contribution']
        if current_pension < 20:  # If contributing less than 20%
            suggested_pension = min(20, 100)  # Suggest up to 20%
            new_taxable = user.annual_income * (1 - suggested_pension/100)
            new_tax = self._calculate_rUK_tax(new_taxable, thresholds)
            tax_saving = calculation['income_tax'] - new_tax
            
            optimizations.append({
                "strategy": "Increase Pension Contributions",
                "current_value": f"{current_pension}% (£{current_pension/100 * user.annual_income:,.2f})",
                "suggested_value": f"{suggested_pension}% (£{suggested_pension/100 * user.annual_income:,.2f})",
                "tax_saving": f"£{tax_saving:,.2f}",
                "net_cost": f"£{suggested_pension/100 * user.annual_income - current_pension/100 * user.annual_income:,.2f}",
                "explanation": f"Increasing pension contributions to {suggested_pension}% would reduce your taxable income and save £{tax_saving:,.2f} in tax. The net cost would be £{suggested_pension/100 * user.annual_income - current_pension/100 * user.annual_income:,.2f} after tax relief."
            })
        
        # 2. Marriage Allowance (if applicable)
        if user.spouse_income is not None and user.spouse_income < thresholds.personal_allowance:
            ...
            optimizations.append({
                ...
                "tax_saving": f"£{tax_saving:,.2f}",
                "net_cost": "£0.00",
                "explanation": f"Transferring £1,260 of your personal allowance to your spouse could save £{tax_saving:,.2f} in tax per year."
            })
        ...
        if user.has_children and user.annual_income > 50000:
            ...
            if user.annual_income > 60000:
                ...
                optimizations.append({
                    ...
                    "suggested_value": "Salary sacrifice to £49,999",
                    "tax_saving": f"£{child_benefit:,.2f}",
                    "net_cost": f"£{user.annual_income - 49999:,.2f}",
                    "explanation": f"Your income is above £60,000, so you're losing the full Child Benefit of £{child_benefit:,.2f} per year. By salary sacrificing £{user.annual_income - 49999:,.2f} into your pension, you could reduce your taxable income to £49,999 and retain the full Child Benefit. The net cost would be £{user.annual_income - 49999:,.2f} - £{child_benefit:,.2f} = £{user.annual_income - 49999 - child_benefit:,.2f} after tax relief and Child Benefit."
                })
            else:
                # Partial clawback
                clawback = (user.annual_income - 50000) / 100 * child_benefit
                optimizations.append({
                    "strategy": "Child Benefit Optimization",
                    ...
                    "net_cost": f"£{user.annual_income - 49999:,.2f}",
                    "explanation": f"Your income is between £50,000 and £60,000, resulting in a partial clawback of £{clawback:,.2f}. By salary sacrificing £{user.annual_income - 49999:,.2f} into your pension, you could reduce your taxable income to £49,999 and retain the full Child Benefit. The net cost would be £{user.annual_income - 49999:,.2f} - £{clawback:,.2f} = £{user.annual_income - 49999 - clawback:,.2f} after tax relief and Child Benefit."
                })
        
        # 4. Personal Allowance Tapering (for incomes between £100,000 and £125,140)
        if 100000 <= user.annual_income < 125140:
            ...
            optimizations.append({
                ...
                "explanation": f"Your income is between £100,000 and £125,140, resulting in a £{reduction:,.2f} reduction in your personal allowance. By salary sacrificing £{sacrifice_needed:,.2f} into your pension, you could reduce your taxable income to £99,999 and retain your full personal allowance. The net cost would be £{net_cost:,.2f} after 40% tax relief. This would save you £{tax_saving:,.2f} in tax and restore your full personal allowance."
            })
        ...
        if user.annual_income > thresholds.basic_rate_threshold:
            ...
            optimizations.append({
                ...
                "explanation": f"Using Gift Aid on a £{donation:,.2f} charitable donation would provide £{tax_relief:,.2f} basic rate tax relief and £{higher_rate_relief:,.2f} higher rate tax relief. The charity would receive £{donation + tax_relief:,.2f}, and you could claim £{higher_rate_relief:,.2f} through your tax return."
            })
